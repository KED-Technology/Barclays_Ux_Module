...
define([
   ...
   */
  class BarclaysWidget extends spandrel((comp, { properties }) => {
    const { selectedSlot } = properties;
    return <div class="BarclaysWidget">
      <div class="BarclaysWidget-header">
        <p>{ TITLE_TEXT }</p>
        <p>
          <span className="BarclaysWidget-selected-slot-text">{ SELECTED_SLOT_TEXT }</span>
          <span className="BarclaysWidget-selected-slot">{ selectedSlot }</span>
        </p>
      </div>
      <div class="BarclaysWidget-content">
        <div className="BarclaysWidget-button-container">
          { comp.getSlots().toArray().map((slot) => (
            <button className="BarclaysWidget-button" type="button" data-slot={ slot.getName() }>
    ...
    getSelectedSlotName() {
      const selectedButton = this.jq().find(
        '.BarclaysWidget-content .' + BUTTON_CLASS + '.' + SELECTED_CLASS);
