define(['baja!', 'jquery', 'Promise', 'nmodule/js/rc/jasmine/promiseUtils', 'nmodule/Barclays/rc/BarclaysWidget'], function (baja, $, Promise, promiseUtils, BarclaysWidget) {
  'use strict';

  const {
    waitForTrue
  } = promiseUtils;
  describe('nmodule/Barclays/rc/BarclaysWidget', () => {
    let stooges, widget, elem;
    ...
    beforeEach(() => {
      ...
      return Promise.all([addStoogesComponent(), initializeWidget()]).then(() => widget.load(stooges));
    });
    afterEach(() => {
    ...
    describe('#doInitialize()', () => {
      it('creates the structure to load buttons into', () => {
        expect(elem.find('.BarclaysWidget-header').length).toBe(1);
        expect(elem.find('.BarclaysWidget-content').length).toBe(1);
      });
      it('arms a handler to set active class on button click', () => {
        var contentDom = elem.find('.BarclaysWidget-content'),
          button = $('<button></button>');
        button.appendTo(contentDom).click();
        expect(button).toHaveClass('active');
      });
      it("removes active class from other buttons when one is clicked", () => {
        const contentDom = elem.find('.BarclaysWidget-content');
      ...
      it('arms a handler to display selected slot name', () => {
        return widget.load(stooges).then(function () {
          const slotDom = elem.find('.BarclaysWidget-selected-slot');
          const contentDom = elem.find('.BarclaysWidget-content');
