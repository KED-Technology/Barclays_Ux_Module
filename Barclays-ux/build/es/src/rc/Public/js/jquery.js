jQuery(function () {
  ...
  async function createBarChart1() {
    ...
    for (const month of last12MonthsData) {
      const scopeUrls = [`${url}/obix/histories/SqlServerDatabase/YearlyScope1/~historyQuery?start=${month.startDate}T23:59:59.000+05:30&limit=1`, `${url}/obix/histories/SqlServerDatabase/YearlyScope2/~historyQuery?start=${month.startDate}T23:59:59.000+05:30&limit=1`, `${url}/obix/histories/SqlServerDatabase/YearlyScope3/~historyQuery?start=${month.startDate}T23:59:59.000+05:30&limit=1`];
  ...
  async function showClusteredBarChart1() {
    ...
    for (const month of last12Months) {
      const scopeUrls = [`${url}/obix/histories/SqlServerDatabase/domesticYearly/~historyQuery?start=${month.startDate}T23:59:59.000+05:30&limit=1`, `${url}/obix/histories/SqlServerDatabase/flushingYearly/~historyQuery?start=${month.startDate}T23:59:59.000+05:30&limit=1`];
      for (let i = 0; i < scopeUrls.length; i++) {
        try {
          const response = await fetch(scopeUrls[i]);
          if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    am4core.ready(async function () {
      ...
      for (const [index, scopeUrl] of urls.entries()) {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          const months = date.toLocaleString('default', {
            month: 'short'
          });
  ...
  const iiaqurl = "https://localhost/obix/config/Drivers/Barclays/IAQ/IAQ/";
  ...
  function startInterval() {
    stopInterval();
    chartInterval = setInterval(() => replaceIndoorChart(createPieChart), 1000);
  ...
  (async function initialize() {
    await createPieChart(); // Create the chart initially
  ...
  async function aqiDonut1() {
    ...
    for (let scope of scopes) {
      try {
        console.log(`https://localhost/obix/config/Drivers/Barclays/IAQ/${scope}`);
        const response = await fetch(`https://localhost/obix/config/Drivers/Barclays/IAQ/${scope}`);
        ...
        // Extract values for ups, hvac, and rtltg
        const ups = xmlDoc.querySelector('real[name="out"]');
        const outValue = parseFloat(ups.getAttribute('display').split(' ')[0]);
  ...
  async function createBarChart() {
    ...
    am4core.ready(async function () {
      ...
      const urlToFetch = `${url}/obix/histories/Barclays_Sup/IAQ$2fA_IAQ_Daily/~historyQuery?start=${startDateValue}T14:33:26.164+05:30&end=${endDateValue}T14:33:26.164+05:30`;
      console.log(urlToFetch);
      try {
        const response = await fetch(urlToFetch);
        console.log(response);
  ...
  function getRandomData() {
    return Array.from({
      length: 12
    }, () => Math.floor(Math.random() * 100) + 20); // Random data for 12 months
  }
