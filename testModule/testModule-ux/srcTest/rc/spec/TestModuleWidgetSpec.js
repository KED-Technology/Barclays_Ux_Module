...
define([
  ...
  'nmodule/testModule/rc/TestModuleWidget' ], function (
  baja,
  $,
  Promise,
  promiseUtils,
  ...
  describe('nmodule/testModule/rc/TestModuleWidget', () => {
    let stooges,
        widget,
        elem;
    ...
    describe('#doInitialize()', () => {
      ...
      it("removes active class from other buttons when one is clicked", () => {
        const contentDom = elem.find('.TestModuleWidget-content');
      ...
      it('arms a handler to display selected slot name', () => {
        return widget.load(stooges)
          .then(function () {
            const slotDom = elem.find('.TestModuleWidget-selected-slot');
            const contentDom = elem.find('.TestModuleWidget-content');
