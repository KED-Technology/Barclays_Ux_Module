...
(function (factory) {
  ...
  var _DataTable = function DataTable(selector, options) {
    // Check if called with a window or jQuery object for DOM less applications
    // This is for backwards compatibility
    if (_DataTable.factory(selector, options)) {
      return _DataTable;
    }

    // When creating with `new`, create a new DataTable, returning the API instance
    if (this instanceof _DataTable) {
  ...
  var _isNumber = function _isNumber(d, decimalPoint, formatted, allowEmpty) {
    var type = _typeof(d);
  ...
  var _htmlNumeric = function _htmlNumeric(d, decimalPoint, formatted, allowEmpty) {
    if (allowEmpty && _empty(d)) {
      return true;
    }
  ...
  var _stripHtml = function _stripHtml(input) {
  ...
  var _flatten2 = function _flatten(out, val) {
  ...
  _DataTable.util = {
    ...
    set: function set(source) {
      if ($.isPlainObject(source)) {
         ...
         */
        return _DataTable.util.set(source._);
      } else if (source === null) {
        ...
        var _setData = function setData(data, val, src) {
        ...
        return function (data, val) {
          ...
          return _setData(data, val, source);
        };
      } else {
    ...
    get: function get(source) {
      if ($.isPlainObject(source)) {
         ...
         */
        var _fetchData = function fetchData(data, type, src) {
              ...
              if (arrayNotation) {
                ...
                if (Array.isArray(data)) {
                  for (var j = 0, jLen = data.length; j < jLen; j++) {
                    out.push(_fetchData(data[j], type, innerSrc));
                  }
                }
        ...
        return function (data, type) {
          ...
          return _fetchData(data, type, source);
        };
      } else {
        // Array or flat object mapping
        return function (data) {
  ...
  function _fnColumnOptions(oSettings, iCol, oOptions) {
    ...
    if (oOptions !== undefined && oOptions !== null) {
      ...
      _fnCamelToHungarian(_DataTable.defaults.column, oOptions, true);
    ...
    if (oCol.mRender && Array.isArray(oCol.mRender)) {
      ...
      var name = copy.shift();
      oCol.mRender = _DataTable.render[name].apply(window, copy);
    }
  ...
  function _fnColumnTypes(settings) {
    ...
    for (i = 0, ien = columns.length; i < ien; i++) {
      ...
      if (renderer && !col._render) {
        col._render = _DataTable.util.get(renderer);
  ...
  function _fnAddData(settings, dataIn, tr, tds) {
    ...
    var rowModel = $.extend(true, {}, _DataTable.models.oRow, {
      src: tr ? 'dom' : 'data',
      idx: rowIdx
    });
   ...
   *  @memberof DataTable#oApi
   */
  var _fnGetObjectDataFn = _DataTable.util.get;
   ...
   *  @memberof DataTable#oApi
   */
  var _fnSetObjectDataFn = _DataTable.util.set;
  ...
  function _fnLayoutDom(settings, dom, insert) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (option == '<') {
        ...
        _DataTable.ext.feature.forEach(function (feature) {
  ...
  function _fnBuildAjax(oSettings, data, fn) {
    ...
    if (typeof ajax === 'function') {
      ...
      var empty = {};
      _DataTable.util.set(ajax.dataSrc)(empty, []);
      callback(empty);
    } else {
   ...
   *  @memberof DataTable#oApi
   */
  var _fnEscapeRegex = _DataTable.util.escapeRegex;
  ...
  function _fnCalculateColumnWidths(settings) {
    ...
    if ((tableWidthAttr || scrollX) && !settings._reszEvt) {
      var bindResize = function bindResize() {
        $(window).on('resize.DT-' + settings.sInstance, _DataTable.util.throttle(function () {
  ...
  function _fnSortFlatten(settings) {
    var i,
      ...
      kLen,
      aSort = [],
      extSort = _DataTable.ext.type.order,
  ...
  function _fnImplementState(settings, s, callback) {
    ...
    var api = settings._bInitComplete ? new _DataTable.Api(settings) : null;
    if (!s || !s.time) {
  ...
  $.extend(_Api2.prototype, {
    ...
    pluck: function pluck(prop) {
      var fn = _DataTable.util.get(prop);
   ...
   * @ignore
   */
  var _table_selector = function __table_selector(selector, a) {
    if (Array.isArray(selector)) {
      var result = [];
      selector.forEach(function (sel) {
        var inner = _table_selector(sel, a);
  ...
  _api_register('tables()', function (selector) {
    ...
    return selector !== undefined && selector !== null ? new _Api2(_table_selector(selector, this.context)) : this;
  });
  ...
  var __details_add = function __details_add(ctx, row, data, klass) {
    ...
    var rows = [];
    var _addRow = function addRow(r, k) {
  ...
  var __details_state = _DataTable.util.throttle(function (ctx) {
    _fnSaveState(ctx[0]);
  }, 500);
  ...
  var __column_header = function __column_header(settings, column, row) {
  ...
  var __column_selector = function __column_selector(settings, selector, opts) {
    var columns = settings.aoColumns,
      ...
      cells = _DataTable.util.get('[].[].cell')(settings.aoHeader),
      nodes = _unique(_flatten2([], cells));
   ...
   */
  _DataTable.use = function (arg1, arg2) {
   ...
   */
  _DataTable.factory = function (root, jq) {
    var is = false;
   ...
   */
  _DataTable.versionCheck = function (version, version2) {
    var aThis = version2 ? version2.split('.') : _DataTable.version.split('.');
   ...
   *    }
   */
  _DataTable.isDataTable = function (table) {
    var t = $(table).get(0);
    var is = false;
    if (table instanceof _DataTable.Api) {
      return true;
    }
   ...
   *    } );
   */
  _DataTable.tables = function (visible) {
    var api = false;
    if ($.isPlainObject(visible)) {
      api = visible.api;
      visible = visible.visible;
    }
    var a = _DataTable.settings.filter(function (o) {
  ...
  _api_register('destroy()', function (remove) {
    ...
    return this.iterator('table', function (settings) {
      ...
      var idx = _DataTable.settings.indexOf(settings);
      if (idx !== -1) {
  ...
  function __mlHelper(localeString) {
    return function (from, to, locale, def) {
      ...
      if (!_DataTable.ext.type.order[typeName + '-pre']) {
        _DataTable.type(typeName, {
  ...
  _DataTable.datetime = function (format, locale) {
    var typeName = 'datetime-' + format;
    if (!locale) {
      locale = 'en';
    }
    if (!_DataTable.ext.type.order[typeName]) {
      _DataTable.type(typeName, {
        detect: function detect(d) {
  ...
  var _extTypes = _DataTable.ext.type;

  // Get / set type
  _DataTable.type = function (name, prop, val) {
    if (!prop) {
      return {
    ...
    var setDetect = function setDetect(detect) {
  ...
  // Get a list of types
  _DataTable.types = function () {
    return _extTypes.detect.map(function (fn) {
      return fn._name;
    });
  };
  var __diacriticSort = function __diacriticSort(a, b) {
  ...
  _DataTable.type('string-utf8', {
    detect: {
      allOf: function allOf(d) {
        return true;
      },
      oneOf: function oneOf(d) {
  ...
  _DataTable.type('html-num-fmt', {
    className: 'dt-type-numeric',
    detect: {
      allOf: function allOf(d, settings) {
        var decimal = settings.oLanguage.sDecimal;
        return _htmlNumeric(d, decimal, true, false);
      },
      oneOf: function oneOf(d, settings) {
  ...
  _DataTable.feature.register = function (name, cb, legacy) {
    _DataTable.ext.features[name] = cb;
    if (legacy) {
      _ext.feature.push({
        cFeature: legacy,
        fnInit: cb
  ...
  _DataTable.feature.register('div', function (settings, opts) {
    var n = $('<div>')[0];
    if (opts) {
  ...
  _DataTable.feature.register('info', function (settings, opts) {
  ...
  // - text
  // - placeholder
  _DataTable.feature.register('search', function (settings, opts) {
    ...
    var jqFilter = $('input', filter).val(previousSearch.search).attr('placeholder', opts.placeholder).on('keyup.DT search.DT input.DT paste.DT cut.DT', searchDelay ? _DataTable.util.debounce(searchFn, searchDelay) : searchFn).on('mouseup.DT', function (e) {
  ...
  _DataTable.feature.register('paging', function (settings, opts) {
    // Don't show the paging input if the table doesn't have paging enabled
    if (!settings.oFeatures.bPaginate) {
      return null;
    }
    opts = $.extend({
      buttons: _DataTable.ext.pager.numbers_length,
  ...
  function _pagingDraw(settings, host, opts) {
    ...
    var plugin = opts.type ? _DataTable.ext.pager[opts.type] : _pagingDynamic,
  ...
  // - menu
  // - text
  _DataTable.feature.register('pageLength', function (settings, opts) {
    var features = settings.oFeatures;
